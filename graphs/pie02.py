...
def pie_artista(_href, df=None):

    df['ARTISTA'] = df['ARTISTA'].str.strip()
    df['ARTISTA'] = df['ARTISTA'].str.replace('Dali', 'Dalì', regex=True)

    counts = {}
    if not df.empty and 'ARTISTA' in df.columns:
        vc = df['ARTISTA'].fillna('N/A').astype(str).value_counts()
        counts = vc.to_dict()


    if not counts:
        fig = px.pie(names=['N/A'], values=[1], title='Opere vendute per Artista')
    ...
    fig = px.pie(names=labels, values=values, title='Opere vendute per Artista', template='plotly_white')
    # compute percentages for legend labels
    total = sum(values) if values else 1
    legend_labels = [f"{lab} ({(val/total)*100:.1f}%)" for lab, val in items]

    # Keep inside slice text as the ARTISTA name and value, but set legend labels to include percent
    ...
    # increase top/bottom margins and height so hover labels near edges are not clipped
